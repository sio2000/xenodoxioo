...
async function main() {
    ...
    data: [
      {
        propertyId: property.id,
        name: 'Private Pool',
        description: 'Infinity pool with sea views',
        icon: 'pool',
      },
      {
        propertyId: property.id,
      ...
      },
      {
        ...
        description: 'Central A/C in all rooms',
        icon: 'wind',
      },
      {
        propertyId: property.id,
  ...
  await prisma.seasonalPricing.upsert({
    where: { propertyId_slug: { propertyId: property.id, slug: 'summer-season' } },
    update: {},
    create: {
      propertyId: property.id,
      name: 'Summer Season',
  ...
  const booking1 = await prisma.booking.create({
    data: {
      ...
      totalPaid: 368.75,
      paymentStatus: 'DEPOSIT_PAID' as any,
      depositPaid: true,
      balancePaid: false,
